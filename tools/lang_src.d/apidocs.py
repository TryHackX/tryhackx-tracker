...
add('', {
    ...
    'apidocs.ab_blocked': ('The report was filed and the torrent was blocked immediately. It is no longer served to anybody.',
        'Zgłoszenie zostało przyjęte, a torrent natychmiast zablokowany. Nie jest już nikomu serwowany.'),
    ...
    'apidocs.abuse_note_review': ('Reports from this key are <strong>held for review</strong>. They land in the operator\'s queue with your name against them, and nothing about the torrent changes until somebody decides.',
        'Zgłoszenia z tego klucza są <strong>wstrzymywane do przeglądu</strong>. Trafiają do kolejki operatora z Waszą nazwą i nic w torrencie się nie zmienia, dopóki ktoś nie zdecyduje.'),
    ...
    'apidocs.block_auto': ('Blocked immediately, without review',
        'Blokada natychmiast, bez przeglądu'),
    'apidocs.block_review': ('Held for review by a person',
        'Wstrzymane do przeglądu przez człowieka'),
    'apidocs.check_body_auto': ('Your key publishes straight to the tracker, so a row is normally live the moment it is accepted. This endpoint answers anyway — a hash can be banned later, and a reconciliation that checks rather than assumes is a reconciliation that stays correct.',
        'Twój klucz publikuje prosto na tracker, więc wpis zwykle jest aktywny od razu po przyjęciu. Ten endpoint i tak odpowiada — hash może zostać później zablokowany, a uzgodnienie, które sprawdza zamiast zakładać, pozostaje poprawne.'),
    ...
    'apidocs.ck_unknown': ('This tracker has no record of that hash.',
        'Ten tracker nie ma takiego hasha.'),
