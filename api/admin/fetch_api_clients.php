...
$st = $db->query("SELECT id, label, key_id, secret_hint, scope, enabled, created_at, last_used_at,
                         last_used_ip, requests_count, auto_approve, required_fields
...
foreach ($st->fetchAll() as $c) {
    ...
    $c['required_fields'] = apiClientCleanFields($c['required_fields']);
    ...
    $c['docs_url'] = apiClientDocsUrl((string)$c['scope'], $c['auto_approve'], $c['required_fields']);
    $clients[] = $c;
}
