...
class Worker:
    ...
    def reset_stale(self):
        try:
            n = self.db.query("UPDATE whitelist SET meta_status='pending', meta_claim=NULL WHERE meta_status='fetching' AND meta_claimed_at < NOW() - INTERVAL %s MINUTE", (self.cfg.stale_minutes,))
            if n:
                log.info("reset %d stale claims", n)
        except Exception as e:
            log.warning("reset_stale: %s", e)

    def claim(self):
        token = secrets.token_hex(8)
        n = self.db.query("UPDATE whitelist SET meta_status='fetching', meta_claim=%s, meta_claimed_at=NOW() WHERE meta_status='pending' ORDER BY meta_priority DESC, meta_requested_at ASC LIMIT 1", (token,))
        if not n:
            return None
        rows = self.db.query("SELECT id, info_hash, magnet_link FROM whitelist WHERE meta_claim=%s", (token,), fetch=True)
        if not rows:
            return None
        row = rows[0]
        row["token"] = token
    ...
    def start(self, row):
        ...
        log.info("fetch #%s %s (%d active)", row["id"], row["info_hash"], len(self.active))
    ...
    def finish(self, token, ok, ti=None, error=None):
        ...
        if ok and ti is not None:
            try:
                ...
                self.db.query("UPDATE whitelist SET name=%s, total_size=%s, files_count=%s, piece_length=%s, meta_status='done', meta_fetched_at=NOW(), meta_error=NULL, meta_claim=NULL WHERE id=%s AND meta_claim=%s",
                              (name, total, count, piece, row["id"], token))
                self.db.query("DELETE FROM whitelist_files WHERE whitelist_id=%s", (row["id"],))
                if files:
                    conn = self.db._connect()
                    with conn.cursor() as cur:
                        cur.executemany("INSERT INTO whitelist_files (whitelist_id, path, size) VALUES (%s, %s, %s)", [(row["id"], p, s) for p, s in files])
                log.info("done #%s %s name=%r size=%d files=%d in %ds", row["id"], row["info_hash"], name, total, count, time.time() - item["started"])
                return
            except Exception as e:
                error = f"store failed: {e}"
                log.warning("store #%s: %s", row["id"], e)
        err = (error or "unknown error")[:255]
        try:
            self.db.query("UPDATE whitelist SET meta_status='failed', meta_error=%s, meta_fetched_at=NOW(), meta_claim=NULL WHERE id=%s AND meta_claim=%s", (err, row["id"], token))
        except Exception as e:
            log.warning("mark failed #%s: %s", row["id"], e)
        log.info("failed #%s %s: %s", row["id"], row["info_hash"], err)
    ...
    def run(self):
        ...
        for token, item in list(self.active.items()):
            try:
                self.db.query("UPDATE whitelist SET meta_status='pending', meta_claim=NULL WHERE id=%s AND meta_claim=%s", (item["row"]["id"], token))
                self.ses.remove_torrent(item["handle"], lt.options_t.delete_files)
            except Exception:
                pass
        log.info("stopped")
